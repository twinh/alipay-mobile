...
功能：把数组所有元素按照固定参数排序，以“参数=参数值”的模式用“&”字符拼接成字符串
输入：Map<String, String> params 需要拼接的数组
输出：String 拼接完成以后的字符串
...
功能：解密
输入：Map<String, String> inputPara 要解密数据
输出：boolean 解密后结果
...
输入：String ALIPAY_GATEWAY_NEW ALIPAY_GATEWAY_NEW
      Map<String, String> sParaTemp 请求参数数组
      String strMethod 提交方式。两个值可选：post、get
      String strButtonName 确认按钮显示文字
输出：String 提交表单HTML文本

public static String buildRequest(String ALIPAY_GATEWAY_NEW, Map<String, String> sParaTemp, String strMethod, String strButtonName, String strParFileName)
...
输入：String ALIPAY_GATEWAY_NEW ALIPAY_GATEWAY_NEW
      String strParaFileName 文件类型的参数名
      String strFilePath 文件路径
      Map<String, String> sParaTemp 请求参数数组
输出：String 支付宝处理结果

public static String getRequestToken(String text) throws Exception
功能：解析远程模拟提交后返回的信息，获得token
输入：String text 要解析的字符串
输出：String 解析结果token
